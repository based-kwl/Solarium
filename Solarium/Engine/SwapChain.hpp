...
namespace Solarium 
{
	...
	public:
		...
		std::vector<vk::Framebuffer> getSwapChainFB() { return swapChainFramebuffers; }
		...
		std::vector<vk::Semaphore> getImageSemaphores() { return imageAvailableSemaphores; }
		std::vector<vk::Semaphore> getFinishedSemaphores() { return renderFinishedSemaphores; }
		size_t getCurrentFrame() { return currentFrame; }
		
		void setCurrentFrame(size_t newFrame) { currentFrame = newFrame; }
		void setImageInFlight(int index, vk::Fence target) { imagesInFlight[index] = target; }
